...
public:
  ...
  static void mult(matrix_aligned<Type, Rows, ColumnsOther, Alg> const &a,
  ...
  auto &get(std::size_t row, std::size_t column) {
    return data_[(row * Columns) + column];
  }

  static std::shared_ptr<matrix_aligned> get_rand_matrix(double max_val,
                                                         double min_val) {
  ...
  static std::shared_ptr<matrix_aligned> get_zeros_matrix() {
    std::shared_ptr<matrix_aligned> ret = std::make_shared<matrix_aligned>();
    for (std::uint32_t i = 0; i < Rows * Columns; ++i) {
      ret->data_[i] = 0;
    }
