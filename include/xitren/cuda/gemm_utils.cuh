...
#pragma unroll
    for (size_t load_idx{0U};
        ...
        //                        [A_thread_block_tile_col_idx] = val;
        // }
        A_thread_block_tile_transposed[A_thread_block_tile_col_idx][A_thread_block_tile_row_idx]
            = val;
    }
// Load data from B on DRAM to B_thread_block_tile on shared memory.
#pragma unroll
    ...
    size_t thread_block_tile_idx, size_t thread_linear_idx, size_t m, size_t n, size_t k)
{
    ...
    static_assert(
        (BLOCK_TILE_SIZE_K + BLOCK_TILE_SKEW_SIZE_K) * sizeof(T) % sizeof(VECTOR_TYPE) == 0U, "");
    static_assert(
        (BLOCK_TILE_SIZE_X + BLOCK_TILE_SKEW_SIZE_X) * sizeof(T) % sizeof(VECTOR_TYPE) == 0U, "");

// Load data from A on DRAM to A_thread_block_tile on shared memory.
#pragma unroll
    for (size_t load_idx{0U};
         load_idx
         < (BLOCK_TILE_SIZE_Y * VECTORIZED_BLOCK_TILE_SIZE_K + NUM_THREADS - 1U) / NUM_THREADS;
         ++load_idx) {
        size_t const A_thread_block_tile_row_idx{(thread_linear_idx + load_idx * NUM_THREADS)
        ...
        if (A_row_idx < m && A_col_idx < k) {
            A_row_vector_vals
                = *reinterpret_cast<VECTOR_TYPE const*>(&A[A_row_idx * lda + A_col_idx]);
        }
...
#pragma unroll
    for (size_t load_idx{0U};
         load_idx
         < (BLOCK_TILE_SIZE_K * VECTORIZED_BLOCK_TILE_SIZE_X + NUM_THREADS - 1U) / NUM_THREADS;
         ++load_idx) {
        size_t const B_thread_block_tile_row_idx{(thread_linear_idx + load_idx * NUM_THREADS)
        ...
        if (B_row_idx < k && B_col_idx < n) {
            B_row_vector_vals
                = *reinterpret_cast<VECTOR_TYPE const*>(&B[B_row_idx * ldb + B_col_idx]);
        }
    ...
    size_t thread_block_tile_idx, size_t thread_linear_idx, size_t m, size_t n, size_t k)
{
    ...
    static_assert((BLOCK_TILE_SIZE_X) * sizeof(T) % sizeof(VECTOR_TYPE) == 0U, "");
    static_assert(
        (BLOCK_TILE_SIZE_Y + BLOCK_TILE_SKEW_SIZE_Y) * sizeof(T) % sizeof(VECTOR_TYPE) == 0U, "");
    static_assert(
        (BLOCK_TILE_SIZE_X + BLOCK_TILE_SKEW_SIZE_X) * sizeof(T) % sizeof(VECTOR_TYPE) == 0U, "");
    ...
    for (size_t load_idx{0U};
        ...
        if (A_thread_block_tile_row_idx < BLOCK_TILE_SIZE_Y
            ...
            for (size_t i{0U}; i < NUM_VECTOR_UNITS; ++i) {
                A_thread_block_tile_transposed[A_thread_block_tile_col_idx + i]
